...
const exRateTxt = document.querySelector("from .result");

// Event listener for currency dropdowns (select)

[fromCur, toCur].forEach((select, i) => {
    for (let curCode in Country_List) {
        const selected = (i === 0 && curCode === "USD") || (i === 1 && curCode === "PKR") ? "selected" : "";
        select.insertAdjacementHTML("beforeend", `<option value="${curCode}" ${selected}>${curCode}</option`);
    }
